<?php 
session_start();
 $_SESSION['url'] = 'vistas2/tbl_tipos_suelos_directos.php';
...
<div class="containertable">
    <div class="d-flex justify-content-between align-items-end mb-4">
        <div>
            <h1 class="poppins-font mb-2">TIPOS SUELOS DIRECTOS</h1>
            <br>
    ...
    <div class="table-responsive">
        ...
        <table class="table table-hover">
            <thead class="table-dark text-center" style="background-color: #343A40;">
                <tr>
                    <th scope="col">Id Suelo Directo</th>
            ...
            <tbody class="text-center">
                ...
                $sql = $conexion->query("SELECT tbl_tipos_suelos_directos.id_suelo_directo,
                tbl_tipos_suelos_directos.nombre_suelo,
                tbl_tipos_suelos_directos.id_Sector,
                tbl_sector.sector, 
                tbl_tipos_suelos_directos.descripcion,
                tbl_tipos_suelos_directos.creado_por,
                tbl_tipos_suelos_directos.fecha_creacion,
                tbl_tipos_suelos_directos.actualizado_por,
                tbl_tipos_suelos_directos.fecha_actualizacion,
                tbl_tipos_suelos_directos.estado
                FROM
                    tbl_tipos_suelos_directos 
                INNER JOIN
                    tbl_sector ON tbl_tipos_suelos_directos.id_Sector = tbl_sector.id_sector");
                while ($datos = $sql->fetch_object()) { ?>
                    <tr>
                        <td><?= $datos->id_suelo_directo ?></td>
                        ...
                        <td>
                            ...
                            ('<?= $datos->id_suelo_directo ?>', '<?= $datos->nombre_suelo ?>', '<?= $datos->sector ?>', '<?= $datos->descripcion ?>', '<?= $datos->creado_por ?>', '<?= $datos->fecha_creacion ?>', '<?= $datos->actualizado_por ?>', '<?= $datos->fecha_actualizacion ?>', '<?= $datos->estado ?>'  )">
                                <i class="bi bi-pencil-square"></i>
                                Editar
                            </button>
                            <form id="deleteForm_<?= $datos->id_suelo_directo ?>" method="POST" action="modelos2/delete_tiposuelo_directo.php" style="display: inline;">
                                <input type="hidden" name="id_suelo_directo" value="<?= $datos->id_suelo_directo ?>">
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="formularioEditar" method="POST" action="modelos2/update_tiposuelo_directo.php">
                    <div class="row">
                    <div class="col-6">
                            <div class="form-group">
                                <label for="id_suelo_directo">Id Suelo </label>
                                <input type="text" class="form-control" id="id_suelo_directo" name="id_suelo_directo" readonly>
                            </div>
                        </div>
                    <div class="col-6">
                            <div class="form-group">
                                <label for="nombre_suelo">Nombre Suelo </label>
...
<div class="modal fade" id="modalForm" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content" role="document">
            <div class="modal-header" style="background-color: #17A2B8;">
                <h5 class="poppins-modal mb-2" id="exampleModalLabel">Crear Tipo Suelo</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Cerrar">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <form action="modelos2/insert_tiposuelo_directo.php" method="POST">
                    <div class="row mb-3">
                    <div class="col-8">
                            <label for="nombre_suelo" class="form-label">Nombre Suelo</label>
                            <input type="text" class="form-control" id="nombre_suelo" name="nombre_suelo" required>
                        </div>
                        <div class="col-8">
...
<script>
    ...
    function abrirModalEditar(id_suelo_directo, nombre_suelo, id_sector, descripcion, creado_por, fecha_creacion, actualizado_por, fecha_actualizacion, estado) {
        document.getElementById("id_suelo_directo").value = id_suelo_directo;
...
<script>
    $(document).ready(function() {
        $("#formularioEditar").on("submit", function(event) {
            ...
            $.ajax({
                url: "modelos2/update_tiposuelo_directo.php",
                method: "POST",
                data: $(this).serialize(),
