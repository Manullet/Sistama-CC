<?php 
session_start();
 $_SESSION['url'] = 'vistas2/tbl_estiercol_fprp_medicion.php';
...
<div class="containertable">
    <div class="d-flex justify-content-between align-items-end mb-4">
        <div>
            <h1 class="poppins-font mb-2">FPRP Medicion</h1>
            <br>
    ...
    <div class="table-responsive">
        ...
        <table class="table table-hover">
            <thead class="table-dark text-center" style="background-color: #343A40;">
                <tr>
                    <th scope="col">Id FPRP estiercol Medicion</th>
                    ...
                    <th scope="col">Promedio Anual Excrecion</th>
                    <th scope="col">Fraccion Excrecion Anual</th>
                    <th scope="col">Emisiones Directas FPRP</th>
            ...
            <tbody class="text-center">
                ...
                while ($datos = $sql->fetch_object()) { ?>
                    <tr>
                        <td><?= $datos->id_fprp_estiercol_medicion ?></td>
                        ...
                        <td><?= $datos->fraccion_excrecion_anual?></td>
                        <td><?= $datos->emisiones_directas_FPRP?></td>
                        <td><?= $datos->hoja?></td>
                        ...
                        <td>
                            ...
                            ('<?= $datos->id_fprp_estiercol_medicion ?>', '<?= $datos->id_ficha ?>', '<?= $datos->categoria_ficha ?>', '<?= $datos->nivel_medicion ?>', '<?= $datos->gestion_estiercol_FPRP ?>', '<?= $datos->sector ?>', '<?= $datos->anio ?>', '<?= $datos->numero_ficha ?>', '<?= $datos->categoria_especie ?>', '<?= $datos->especie ?>', '<?= $datos->categoria_medicion ?>', '<?= $datos->criterio_medicion ?>', '<?= $datos->cantidad_especie ?>', '<?= $datos->promedio_anual_excrecion ?>' ,'<?= $datos->fraccion_excrecion_anual ?>', '<?= $datos->emisiones_directas_FPRP ?>', '<?= $datos->hoja ?>', '<?= $datos->referencia ?>' ,'<?= $datos->descripcion ?>', '<?= $datos->sigla ?>', '<?= $datos->estado ?>' )">
                                <i class="bi bi-pencil-square"></i>
                                Editar
                            </button>
                            <form id="deleteForm" method="POST" action="modelos2/delete_fprp_medicion.php" style="display: inline;">
                                <input type="hidden" name="id_fprp_estiercol_medicion" value="<?= $datos->id_fprp_estiercol_medicion ?>">
...
<div class="modal fade" id="modalEditar" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        ...
        <h5 class="modal-title">EDITAR 3C4 FPRP</h5>
      ...
      <form id="formularioEditar" method="POST" action="modelos2/update_fprp_medicion.php">
        ...
        <div class="tab-content">
          <div class="tab-pane active" id="sistemaTab">
                        ...
                        <div class="col-md-6">
                        <label for="id_fprp_estiercol_medicion" class="form-label">Id FPRP Estiercol Medicion</label>
                                <input type="text" class="form-control" id="id_fprp_estiercol_medicion" name="id_fprp_estiercol_medicion" readonly>
                        </div>
        ...
        ?>
                        ...
                        <div class="col-md-6">
                            <label for="id_gestion_estiercol_FPRP" class="form-label">Gestion Estiercol FPRP</label>
                            <select class="form-control" id="id_gestion_estiercol_FPRP_edit" name="id_gestion_estiercol_FPRP" required>
        <?php
        // Conexión a la base de datos
        include '../php/conexion_be.php';

        // Consulta SQL para obtener los valores disponibles de ID y Nombre del sistema
        $sql = "SELECT id_gestion_estiercol_FPRP, gestion_estiercol_FPRP FROM tbl_estiercol_fprp_n";
        ...
        if (mysqli_num_rows($result) > 0) {
            while ($row = mysqli_fetch_assoc($result)) {
                ...
                echo '<option value="' . $row["id_gestion_estiercol_FPRP"] . '">' . $row["gestion_estiercol_FPRP"] . '</option>';
            }
        } else {
            echo '<option value="">No hay campos disponibles</option>';
        }
        ...
        ?>
          ...
          <div class="tab-pane fade" id="datosTab">
                       <div class="col-md-6">
                        ...
                        <div class="col-md-6">
                        <label for="promedio_anual_excrecion" class="form-label">Promedio Anual Excrecion</label>
                                <input type="text" class="form-control" id="promedio_anual_excrecion_edit" name="promedio_anual_excrecion" required>
                        </div>
  ...
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      <div class="modal-header" style="background-color: #17A2B8; color: white;">
        <h5 class="modal-title">AGREGAR ESTIERCOL FPRP MEDICION</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <div class="modal-body">
      <form action="modelos2/insert_fprp_medicion.php" method="POST">
        <ul class="nav nav-tabs">
          <li class="nav-item">
        ...
        <div class="tab-content">
          ...
          <div class="col-md-6">
                    <label for="id_gestion_estiercol_FPRP">Gestion Estiercol FPRP</label>
                                <select class="form-control" id="id_gestion_estiercol_FPRP" name="id_gestion_estiercol_FPRP" required>
                                <?php
                                // Conexión a la base de datos
                                include '../php/conexion_be.php';
          ...
          <div class="tab-pane fade" id="cantidadesTab">
                    <div class="col-md-6">
                            ...
                            <div class="col-md-6">
                                <label for="promedio_anual_excrecion" class="form-label">Promedio Anual Excrecion</label>
                                <input type="text" class="form-control" id="promedio_anual_excrecion" name="promedio_anual_excrecion" required>
                            </div>
                            <div class="col-md-6">
                                <label for="fraccion_excrecion_anual" class="form-label">Fraccion Excrecion Anual</label>
                                <input type="text" class="form-control" id="fraccion_excrecion_anual" name="fraccion_excrecion_anual" required>
                            </div>
                            <div class="col-md-6">
                                <label for="hoja" class="form-label">Hoja</label>
...
<script>
    ...
    function abrirModalEditar(id_fprp_estiercol_medicion, id_ficha, id_categoria_ficha, nivel_medicion, id_gestion_estiercol_FPRP, id_sector, anio, numero_ficha, id_categoria_especie, id_especie, categoria_medicion, id_criterios_medicion, cantidad_especie, promedio_anual_excrecion ,fraccion_excrecion_anual, emisiones_directas_FPRP, hoja, referencia, descripcion, sigla ,estado) {
        document.getElementById("id_fprp_estiercol_medicion").value = id_fprp_estiercol_medicion;
        ...
        document.getElementById("id_gestion_estiercol_FPRP_edit").value = id_gestion_estiercol_FPRP;
        ...
        document.getElementById("promedio_anual_excrecion_edit").value = promedio_anual_excrecion;
        document.getElementById("fraccion_excrecion_anual_edit").value = fraccion_excrecion_anual;
        document.getElementById("emisiones_directas_FPRP_edit").value = emisiones_directas_FPRP;
...
<script>
    $(document).ready(function() {
        $("#formularioEditar").on("submit", function(event) {
            ...
            $.ajax({
                url: "modelos2/update_fprp_medicion.php",
                method: "POST",
                data: $(this).serialize(),
